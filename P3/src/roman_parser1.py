...
# Gramática G1 (left-recursive for Low*):
# Roman -> Hundreds Tens Units (here named romanNumber with thousand,hundred,ten,digit)
...
# LowHundreds (left-recursive): LowHundreds -> LowHundreds C | lambda
def p_low_hundreds_rec(p):
    """low_hundreds : low_hundreds C"""
    left = p[1]
    if not left["valid"]:
        p[0] = {"valid": False, "val": -1, "count": left.get("count", 0)}
    else:
        cnt = left.get("count", 0) + 1
        if cnt > 3:
            p[0] = {"valid": False, "val": -1, "count": cnt}
        else:
            p[0] = {"valid": True, "val": left.get("val", 0) + 100, "count": cnt}
...
def p_small_hundred(p):
    """small_hundred : C C C
                      | C C
                      | C"""
    # keep for compatibility (rarely used in G1 but safe)
    if len(p) == 4:
...
# LowTens (left-recursive): low_tens -> low_tens X | lambda
def p_low_tens_rec(p):
    ...
    if not left["valid"]:
        ...
        if cnt > 3:
            ...
            p[0] = {"valid": True, "val": left.get("val", 0) + 10, "count": cnt}
...
# LowUnits (left-recursive): low_units -> low_units I | lambda
def p_low_units_rec(p):
    ...
    if not left["valid"]:
        ...
        if cnt > 3:
            ...
            p[0] = {"valid": True, "val": left.get("val", 0) + 1, "count": cnt}
